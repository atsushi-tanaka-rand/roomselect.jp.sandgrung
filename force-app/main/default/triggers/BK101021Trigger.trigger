trigger BK101021Trigger on PortalFavoritePropertyDifference__c (before insert, after insert, before update, after update, 
before delete, after delete, after undelete) {
    SystemConfig__c config = SystemConfig__c.getOrgDefaults();
    if (config.IsSkipTrigger__c) {
        return;   // フラグONなら処理Skip
    }
    BK101021TriggerHandler handler = new BK101021TriggerHandler(Trigger.isExecuting, Trigger.size);
