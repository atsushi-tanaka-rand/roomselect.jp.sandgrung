trigger BK101020Trigger on BrowsingHistoryDifference__c (before insert, after insert, before update, after update, 
before delete, after delete, after undelete) {
    SystemConfig__c config = SystemConfig__c.getOrgDefaults();
    if (config.IsSkipTrigger__c) {
        return;   // フラグONなら処理Skip
    }
    BK101020TriggerHandler handler = new BK101020TriggerHandler(Trigger.isExecuting, Trigger.size);
