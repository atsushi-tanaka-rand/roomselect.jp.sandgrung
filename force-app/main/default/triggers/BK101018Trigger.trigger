// 本会員差分トリガ
trigger BK101018Trigger on MemberDifference__c (before insert, after insert, before update, after update, 					
 before delete, after delete, after undelete) {
    SystemConfig__c config = SystemConfig__c.getOrgDefaults();					
    if (config.IsSkipTrigger__c) {					
        return;   // フラグONなら処理Skip					
    }					     
    BK101018TriggerHandler handler = new BK101018TriggerHandler(Trigger.isExecuting, Trigger.size);
