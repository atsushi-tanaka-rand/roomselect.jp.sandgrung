trigger BK101025Trigger on SearchHistoryDifference__c (before insert, after insert, before update, after update, 
before delete, after delete, after undelete) {
    SystemConfig__c config = SystemConfig__c.getOrgDefaults();
    if (config.IsSkipTrigger__c) {
        return;   // フラグONなら処理Skip
    }
    BK101025TriggerHandler handler = new BK101025TriggerHandler(Trigger.isExecuting, Trigger.size);
