// ポータルサイト閲覧差分トリガ
trigger BD401007Trigger on PortalSiteBrowseDifference__c (before insert, after insert, before update, after update, 					
 before delete, after delete, after undelete) {
    SystemConfig__c config = SystemConfig__c.getOrgDefaults();					
    if (config.IsSkipTrigger__c) {					
        return;   // フラグONなら処理Skip					
    }					     
    BD401007TriggerHandler handler = new BD401007TriggerHandler(Trigger.isExecuting, Trigger.size);
